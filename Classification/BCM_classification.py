...
model.fit(variables(0), variables(1))

#PREDICTION

def predict(X,y,x_predict):
    while type(x_predict) == int and x_predict < len(variables(3)):
        if True:
            predict = model.predict(variables(2).values[x_predict].reshape(1, -1), \
                                    y=np.zeros_like(variables(3)[x_predict].reshape(1, -1)))
            ...
            ax1.set_title('Image from the MNIST dataset:{:d}'.format(variables(3)[x_predict].argmax()))
            ax1.imshow(variables(2).values[x_predict].reshape(28, 28), cmap='gray'); ax1.axis('off')
        ...
        print("Error: x_predict must be an integer number lower than", len(variables(3))-1,\
              ". Please, enter a valid number next time")

predict(variables(2), variables(3), x_predict=0)

######## EVALUATION OF THE ACCURACY

def testing(x,y):

    testing = model.predict(variables(2), variables(3))

    y_values = [model.weights[np.argmax(x)][28*28:].argmax() for x in testing]

    y_true = variables(3).argmax(axis=1)
    y_pred = np.asarray(y_values)

    print('Prediction Accuracy on the test set: {:.3f}'.format(accuracy_score(y_true, y_pred)))

testing(variables(2), variables(3))